// ai-regulation-tracker — scraper module
...
export class Scraper extends EventEmitter {
  ...
  constructor(options: ScraperOptions = {}) {
    super();
    this.options = { name: options.name ?? "Scraper", enabled: options.enabled ?? true, timeout: options.timeout ?? 30000 };
  }
  ...
  async process(input: unknown): Promise<ScraperResult> {
